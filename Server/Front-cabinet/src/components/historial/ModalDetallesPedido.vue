...
<template>
    ...
    >
        <div v-if="pedido" class="p-3">
            ...
            <div class="border-top-1 border-gray-600 pt-3">
                ...
                <ul class="list-none p-0 m-0">
                    <li v-for="h in pedido.herramientas" :key="h.codigo" class="surface-100 p-4 mb-3 border-round shadow-1">
                        <div class="flex justify-content-between mb-2 border-bottom-1 border-gray-700 pb-2">
                            <div>
                        ...
                        <div v-if="h.historialDevoluciones && h.historialDevoluciones.filter(d => d.cantidad > 0).length > 0" class="mt-3 p-3 surface-200 border-round border-1 border-gray-700">
                            ...
                            <template v-for="(dev, i) in h.historialDevoluciones" :key="i">
                                <div v-if="dev.cantidad > 0" class="text-sm text-300 flex justify-content-between py-2 border-bottom-1 border-gray-700 last-border-none align-items-center">
                                    <span>
                                    ...
                                    <span class="text-500"><i class="pi pi-calendar mr-1"></i>{{ formatearFecha(dev.fecha) }}</span>
                                </div>
                            </template>
                        </div>
        ...
        <template #footer>
            <Button label="Cerrar Detalles" icon="pi pi-times" @click="emit('cerrar')" class="p-button-text text-white hover:text-blue-300 transition-colors" />
        </template>
    </Dialog>
</template>
